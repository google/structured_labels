...
def corrupt_mnist(x, y, py1_y0, pflip0=.1, pflip1=.1, npix=5, rng=None):
	...
	if pflip1 > 0:
		flips = rng.choice(
			range(y1.shape[0]), size=int(pflip1 * y1.shape[0]), replace=False)
		y1[flips] = 1 - y1[flips]
...
def get_corrupt_minst(p_tr=.7,
	...
	# in_sample = rng.choice(x_train_valid_or.shape[0], size=5000, replace=False).tolist()
	# out_sample = list(set(range(x_train_valid_or.shape[0])) - set(in_sample))

	# x_train_valid_aug = x_train_valid_or[out_sample]
	# y_train_valid_aug = y_train_valid_or[out_sample]


	# x_train_valid_or = x_train_valid_or[in_sample]
	# y_train_valid_or = y_train_valid_or[in_sample]
...
def build_input_fns(p_tr=.7,
	...
	if oracle_prop > 0.0:
		experiment_directory = f'{DATA_DIR}/experiment_data/ors{random_seed}_{py1_y0_s[0]}'
	else:
		experiment_directory = f'{DATA_DIR}/experiment_data/rs{random_seed}'

	if not os.path.exists(f'{experiment_directory}/train.pkl'):
		...
		pickle.dump(train_data, open(f'{experiment_directory}/train.pkl', 'wb'))
		pickle.dump(valid_data, open(f'{experiment_directory}/valid.pkl', 'wb'))

		for py1_y0_s_val, py1_y0_s_data in shifted_data_dict.items():
			pickle.dump(py1_y0_s_data,
				open(f'{experiment_directory}/test_shift{py1_y0_s_val}.pkl', 'wb'))

	# Build an iterator over training batches.
	def train_input_fn(params):
		batch_size = params['batch_size']
		train_data = pickle.load(open(f'{experiment_directory}/train.pkl', 'rb'))
	...
	def valid_input_fn(params):
		batch_size = params['batch_size']
		valid_data = pickle.load(open(f'{experiment_directory}/valid.pkl', 'rb'))
	...
	def eval_input_fn_creater(py):
		shifted_test_data = pickle.load(
			open(f'{experiment_directory}/test_shift{py}.pkl', 'rb'))
