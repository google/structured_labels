...
def get_predictions(rs_config, py1_y0_s, base_dir, save_predictions=False):
	...
	if save_predictions:
		...
		pred_df.to_csv(f'{base_dir}/final_models/predictions_DNN_rs{random_seed}.csv', index=False)
	# --- get full auroc
	auroc_full = roc_auc_score(pred_df.y, pred_df.predictions)

	# --- get metrics for v = 0
	pred_df_v0 = pred_df[(pred_df.v == 0)]
	auroc_v0 = roc_auc_score(pred_df_v0.y, pred_df_v0.predictions)

	acc_v0 = np.mean((pred_df_v0.y ==  pred_df_v0.pred_class) * 1.0)

	pred_df_v00 = pred_df[((pred_df.v == 0) & (pred_df.y == 0))]
	acc_00 = 1.0 - np.mean(pred_df_v00.pred_class)
	del pred_df_v00

	pred_df_v01 = pred_df[((pred_df.v == 0) & (pred_df.y == 1))]
	acc_01 = np.mean(pred_df_v01.pred_class)
	del pred_df_v01

	# --- get metrics for v = 1
	pred_df_v1 = pred_df[(pred_df.v == 1)]
	auroc_v1 = roc_auc_score(pred_df_v1.y, pred_df_v1.predictions)
	acc_v1 = np.mean((pred_df_v1.y ==  pred_df_v1.pred_class) * 1.0)

	pred_df_v10 = pred_df[((pred_df.v == 1) & (pred_df.y == 0))]
	acc_10 = 1.0 - np.mean(pred_df_v10.pred_class)
	del pred_df_v10

	pred_df_v11 = pred_df[((pred_df.v == 1) & (pred_df.y == 1))]
	acc_11 = np.mean(pred_df_v11.pred_class)
	del pred_df_v11
...
def main(model, dataset, experiment_name, xv_method, pval, batch_size,
	clean_back, save_predictions):
	...
	all_results = []
	runner_wrapper = functools.partial(get_predictions, py1_y0_s=0.5,
		base_dir=base_dir, save_predictions=save_predictions)
	...
	all_results = all_results.agg(['mean', 'std'])
	print(all_results[['random_seed', 'acc_v0', 'acc_v1']])

	if dataset == 'waterbirds':
		all_results.to_csv(
			(f'{final_models_dir}/v_performance_{model}_{xv_method}_{experiment_name}'
				f'_{clean_back}_{batch_size}.csv')
		)
	else:
		all_results.to_csv(
...
if __name__ == "__main__":
	...
	parser.add_argument('--save_predictions', '-save_predictions',
		action='store_true',
		default=False,
		help="Should I save predictions for the model (only for DNNS)")
