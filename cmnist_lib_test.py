...
"""Tests for structured_labels.cmnist_lib."""
...
class CmnistLibTest(absltest.TestCase):
  ...
  def test_color_corrupt_1(self):
    ...
    xc1 = cmnist_lib.color_corrupt_img(self.img, 1, 1e5)
    xc1_mask = xc1[:, :, 2] > .5
  ...
  def test_color_corrupt_0(self):
    ...
    xc0 = cmnist_lib.color_corrupt_img(self.img, 0, 1e5)
    xc0_mask = xc0[:, :, 1] > .5
  ...
  def test_corrupt_mnist_noiseless_1(self):
    ...
    _, y_lab = cmnist_lib.corrupt_mnist(
        self.x, self.y, py1_y0=1, pflip=0, npix=5)
  ...
  def test_corrupt_mnist_noisy_1(self):
    ...
    _, y_lab = cmnist_lib.corrupt_mnist(
        self.x, self.y, py1_y0=1, pflip=.1, npix=5)
  ...
  def test_corrupt_mnist_noiseless_0(self):
    _, y_lab = cmnist_lib.corrupt_mnist(
        self.x, self.y, py1_y0=0, pflip=0, npix=5)
  ...
  def test_corrupt_mnist_noisy_0(self):
    _, y_lab = cmnist_lib.corrupt_mnist(
        self.x, self.y, py1_y0=0, pflip=.1, npix=5)
