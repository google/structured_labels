...
FLAGS = flags.FLAGS
flags.DEFINE_string('experiment_name', 'skew_train', 'Name of the experiment.')
...
def main(argv):
	...
	for model_id, model in enumerate(model_to_spec):
		...
		try:
			model_res = pd.read_csv((f'{BASE_DIR}/final_models/v_performance_{model}_'
				f'{FLAGS.experiment_name}.csv'),
			index_col=0)
			print(model_res)
	...
	if FLAGS.metric == 'auc':
		legend_elements = [  # noqa: F841
			Line2D([0], [0], marker='s', color='#C85200', label='Men',
				lw=0, markerfacecolor='#C85200'),
			Line2D([0], [0], marker='s', color='#A2C8EC', label='Women',
				lw=0, markerfacecolor='#A2C8EC')
		]
	else:
		legend_elements = [  # noqa: F841
			Line2D([0], [0], marker='s', color='#C85200', label='Healthy women',
				lw=0, markerfacecolor='#C85200'),
			Line2D([0], [0], marker='s', color='#A2C8EC', label='Pneumonia women',
				lw=0, markerfacecolor='#A2C8EC'),
			Line2D([0], [0], marker='s', color='#5F6B29', label='Healthy men',
				lw=0, markerfacecolor='#5F6B29'),
			Line2D([0], [0], marker='s', color='#006BA4', label='Pneumonia men',
	...
	savename = os.path.join(results_dir,
		(f'chexpert_{FLAGS.experiment_name}_{FLAGS.pval}_'
