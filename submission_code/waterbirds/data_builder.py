...
WATER_IMG_DIR_CLEAN = 'water_clean'
LAND_IMG_DIR_CLEAN = 'land_clean'
...
def read_decode_jpg(file_path):
	"""helper function to read in jpgs"""
...
def read_decode_png(file_path):
	"""helper function to read in pngs"""
	img = tf.io.read_file(file_path)
...
def decode_number(label):
	"""helper function to convert label from string to number"""
...
def map_to_image_label(x, pixel):
	"""maps input string to images, and labels"""
	...
	if weights_included:
		balanced_weights_pos = x[5]
		balanced_weights_neg = x[6]
		balanced_weights_both = x[7]
...
def get_bird_type(x):
	""" returns 1 if waterbird, 0 otherwise"""
	bird_type = [
...
def get_weights(data_frame):
	...
	# aggregate weights
	data_frame.drop(['weights_pos', 'weights_neg'], axis=1, inplace=True)
...
def create_save_waterbird_lists(experiment_directory, main_directory, p_tr=.7,
	...
	# TODO make the validation/train split a function parameter
